...
const UpdatePage = () => {
    ...
    const DetailProfile = async () => {
        ...
        try {
            ...
            response = await auth.profileDetail(id.id);
            console.log(response)
            console.log(response.data)
            data = response.data
            setProfile(data)
            let arr = removeOddIndexes(data.technicalSkills)
            let skillarr = data.skills.split(',')
            for(let i = 0; i < arr.length; i++){
                toggleTechnicalSkills(skillarr[i], arr[i])
            }
            console.log(data)
        } catch (error) {
...
// 값 변경 핸들러
    ...
    const updateProfile = async (e) => {
        ...
        try {
            ...
            response = await auth.profileUpdate(profile); // axios 내부적으로 사용될 수 있음
    ...
    const formFields = [
        { label: '이름', type: 'text', placeholder: '이름을 입력하세요', name: 'name', required: true, value: profile.name },
        { label: '전화번호', type: 'number', placeholder: '전화번호를 입력하세요', name: 'number', required: true, value: profile.number },
        { label: '나이', type: 'number', placeholder: '나이를 입력하세요', name: 'age', required: true, value: profile.age },
            ...
            <Form.Group controlId={`formBasic${field.name}`} key={index}>
                ...
                {field.type === 'select' ? (
                    <Form.Select
                        ...
                        name={field.name}
                        id={field.name}
                        value={profile[field.name] || field.value}
